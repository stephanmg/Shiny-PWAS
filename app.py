...
from backend import (
    parse_gene_list, resolve_gene, fetch_gene_results,
    tidy_table, top_n_per_gene
)
...
def server(input, output, session):
    ...
    def load_phenos():
        ...
        if not genes:
            ...
            df_results.set(pd.DataFrame()); return
        frames = []
        for g in genes:
            ensg, sym = resolve_gene(g)
            if not ensg:
                log(f"! Could not resolve '{g}' — skipping."); continue
            try:
                df = fetch_gene_results(ensg, subset)
                if not df.empty:
                    df = df.copy(); df["gene"] = sym
                    frames.append(df); log(f"{sym}: {len(df)} rows.")
                else:
                    log(f"{sym}: 0 rows.")
        ...
        if frames:
            ...
            df_results.set(all_df); log(f"Total rows combined: {len(all_df)}")
        else:
            df_results.set(pd.DataFrame()); log("No data loaded.")
    ...
    def plot_out():
        df = df_results.get()
        if df is None or df.empty:
            fig, ax = plt.subplots(); ax.text(0.5,0.5,"No data yet. Click 'Load phenotypes'.",
                                              ha="center", va="center"); ax.set_axis_off(); return fig
        metric = input.metric(); use_log = bool(input.neglog10()); limit = max(1, int(input.limit()))
        d = prepare_plot_df(df, metric, limit)
        if d.empty:
            fig, ax = plt.subplots(); ax.text(0.5,0.5,"No rows to plot.", ha="center", va="center"); ax.set_axis_off(); return fig
        ...
        ax.set_title(f"{'−log10('+metric+')' if use_log else metric} by analysis type (top {limit}/gene/group)")
        if input.show_legend(): ax.legend(title="Gene", loc="upper right", frameon=True, fontsize="small")
        fig.tight_layout(); return fig

    # tables: top N per gene, per endpoint
    def _tbl(kind: str):
        df = df_results.get()
        if df is None or df.empty: return tidy_table(pd.DataFrame())
        sub = top_n_per_gene(df, kind, int(input.limit()))
        return tidy_table(sub)

    @output
    @render.table
    def tbl_continuous(): return _tbl("CONTINUOUS_VARIABLE")

    @output
    @render.table
    def tbl_cv(): return _tbl("CV_ENDPOINTS")

    @output 
    @render.table
    def tbl_self(): return _tbl("SELF_REPORTED")

    @output 
    @render.table
    def tbl_phecode(): return _tbl("PHECODES")
