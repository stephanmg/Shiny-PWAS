...
def resolve_gene(name: str) -> tuple[str|None, str|None]:
    ...
    try:
        if name.upper().startswith("ENSG"):
            r = requests.get(f"{API}/gene/ensembl/{name}", timeout=30); r.raise_for_status()
            j = r.json()
            return (j.get("ensembl_id") or name, j.get("symbol") or name)
        r = requests.get(f"{API}/gene/name/{name}", timeout=30); r.raise_for_status()
        j = r.json()
...
def outcome_catalog() -> pd.DataFrame:
    r = requests.get(f"{API}/outcome", timeout=60); r.raise_for_status()
...
def enrich_labels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "outcome_id" not in df.columns:
        if "outcome" in df.columns: df = df.rename(columns={"outcome":"outcome_id"})
        elif "id" in df.columns:   df = df.rename(columns={"id":"outcome_id"})
...
def tidy_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "p" in m.columns: out["p"] = m["p"]
    if "q" in m.columns: out["q"] = m["q"]
    return out
...
def top_n_per_gene(df: pd.DataFrame, analysis_type: str, n: int) -> pd.DataFrame:
    ...
    if "gene" not in sub.columns: sub["gene"] = "unknown"
